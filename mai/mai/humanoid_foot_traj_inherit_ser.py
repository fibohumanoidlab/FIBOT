...
class FootTrajPublisher():
    # class FootTrajPublisher no inherrit from Node. 
    # because SendFWService(Node) already have super().__init__('send_xyz_service')

    # node:Node = node comes from Node Library and class SendFWService(Node)
    # However, you can write only node but function from rclpy module is not highlight

    def __init__(self,node:Node, hip_input,foot_input):
        # super().__init__('foot_traj_publisher')
        self.node=node
        self.publisher_ = self.node.create_publisher(Float32MultiArray, 'foot_traj_topic', 10)
       
        timer_period = 0.01  # seconds
        self.timer = self.node.create_timer(timer_period, self.timer_callback)

        self.timestamp_foot = self.node.get_clock().now().nanoseconds # loop control (ns)
        ...
        self.timestamp_hip = self.node.get_clock().now().nanoseconds # loop control (ns)
    ...
    def foot_xyz_traj(self, foot_input):
        # x_start,x_stop,y_start,y_stop,z_start,z_mid,z_stop,time_middle,time_walking,isRight
        if (self.node.get_clock().now().nanoseconds - self.timestamp_foot >= 5000000): # 5 ms
            self.timestamp_foot = self.node.get_clock().now().nanoseconds
            if self.initial_foot:
                self.t_start_foot = self.node.get_clock().now().nanoseconds
                self.t_m_foot = foot_input[7] # s
                self.t_s_foot = foot_input[8] # s
                self.initial_foot = 0
                
            else:
                t_cont = (self.node.get_clock().now().nanoseconds - self.t_start_foot)*1e-9 # s
                ...
                if int(foot_input[9]):
                    ...
                    self.foot_y_traj = foot_input[2]
                if (self.node.get_clock().now().nanoseconds - self.t_start_foot)*1e-9 <= self.t_m_foot:
                ...
                elif (self.node.get_clock().now().nanoseconds - self.t_start_foot)*1e-9 > self.t_m_foot and (self.node.get_clock().now().nanoseconds - self.t_start_foot)*1e-9 <= self.t_s_foot:
                ...
                elif ((self.node.get_clock().now().nanoseconds - self.t_start_foot)*1e-9 > self.t_s_foot) :
                    self.initial_foot = 1
        return [float(self.foot_x_traj), float(self.foot_y_traj), float(self.foot_z_traj)]
    
    def hip_xyz_traj(self, hip_input):
        # x_start,x_mid,x_stop,vx_start,vx_mid,vx_stop,accx_start,y_start,y_stop,z_start,z_stop,vz_start,vz_stop,time_middle,time_walking
        if (self.node.get_clock().now().nanoseconds - self.timestamp_hip >= 5000000): # 5 ms
            self.timestamp_hip = self.node.get_clock().now().nanoseconds
            if self.initial_hip:
                self.t_start_hip = self.node.get_clock().now().nanoseconds
                self.t_m_hip = hip_input[13] # s
                self.t_s_hip = hip_input[14] # s
                self.initial_hip = 0
                
            else:
                t_cont = (self.node.get_clock().now().nanoseconds - self.t_start_hip)*1e-9 # s
                ...
                if (self.node.get_clock().now().nanoseconds - self.t_start_hip)*1e-9 <= self.t_m_hip:
                ...
                elif (self.node.get_clock().now().nanoseconds - self.t_start_hip)*1e-9 > self.t_m_hip and (self.node.get_clock().now().nanoseconds - self.t_start_hip)*1e-9 <= self.t_s_hip:
                ...
                elif ((self.node.get_clock().now().nanoseconds - self.t_start_hip)*1e-9 > self.t_s_hip) :
                    self.initial_hip = 1
